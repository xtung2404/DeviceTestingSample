...
import com.example.devicetestingsample.UI.adapter.TestingDevicesAdapter
...
//    val connectedDevices = arrayListOf<IoTDevice>()
    val connectedDevices = arrayListOf<Pair<IoTDevice, Boolean>>()
    ...
    private val connectedDevicesAdapter: ConnectedDevicesAdapter by lazy {
        ConnectedDevicesAdapter(
            onDeleteDevice = {deletedDev ->
                SmartSdk.deviceHandler().delete(
                    deletedDev.uuid,
                    object : RequestCallback<Boolean> {
                        override fun onSuccess(p0: Boolean?) {
                            CoroutineScope(Dispatchers.Main).launch {
                                val index = connectedDevices.indexOfFirst {
                                    it.first.uuid == deletedDev.uuid
                                }
                                if (index != -1) {
    ...
    private val testingDevicesAdapter: TestingDevicesAdapter by lazy {
        TestingDevicesAdapter()
    }
    ...
    override fun initVariable() {
        ...
        binding.apply {
            ...
            sharedPrefs?.let {
                ...
                edtDevTesting.setText(it.getInt(KEY_NUMBER_DEVICE_TESTING).toString())
            }
            if (edtProductid.text.isNullOrEmpty()) {
                edtProductid.setText("726738018C040002")
            }
            if (edtDevTesting.text.contentEquals("0")) {
                edtDevTesting.setText("5")
            }
        }
    }
    ...
    override fun initAction() {
        ...
        binding.apply {
            ...
            edtDevTesting.addTextChangedListener(object : TextWatcher {
                override fun beforeTextChanged(
                    s: CharSequence?,
                    start: Int,
                    count: Int,
                ...
                override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
                    s?.let { wifiPass ->
                        sharedPrefs?.setInt(KEY_NUMBER_DEVICE_TESTING, if (wifiPass.isEmpty()) 0 else wifiPass.toString().toInt()
                        )
                    }
                }
            ...
            btnStartScan.setOnClickListener {
                if (isScanning) {
                    binding.btnStartScan.text = context?.resources?.getString(R.string.start_scanning_for_devices)
                    resetUI()
                    stopScan()
                    isScanning = false
                } else {
    ...
    fun startScan() {
        binding.btnStartScan.text = context?.resources?.getString(R.string.stop_scanning)
            ...
            object : SetupDeviceWileCallback {
                ...
                override fun onSuccess(ioTDevice: IoTDevice?) {
                    CoroutineScope(Dispatchers.Main).launch {
                        ...
                        ioTDevice?.let {
                            connectedDevices.add(0, Pair(it, false))
                            connectedDevicesAdapter.notifyItemInserted(0)
                            binding.txtNumberConnectedDevices.text = context?.resources?.getString(R.string.number_of_connected_devices) + ": ${connectedDevices.size}/${binding.edtDevTesting.text}"
                            if (connectedDevices.size == binding.edtDevTesting.text.toString().toInt()) {
                                binding.btnStartScan.text = context?.resources?.getString(R.string.start_scan)
                                resetUI()
                                stopScan()
                                return@launch
                            }
    ...
    private fun setUpDialogTestingIr() {
        ...
        btnOff.setOnClickListener {
            ioTDeviceRemote?.let {remote ->
                SmartSdk.controlHandler().controlDevicePower(
                    remote.uuid,
                    false,
                    null
                )
                connectedDevicesAdapter.updateCheckState(remote.rootUuid)
            }
...
//    }
    companion object {
        ...
        val KEY_NUMBER_DEVICE_TESTING = "NUMBER_DEVICE_TESTING"
    }
